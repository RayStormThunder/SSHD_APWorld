...
# Handle KivyMD version compatibility (1.x vs 2.x)
try:
    from kivymd.uix.divider import MDDivider
except ImportError:
    # KivyMD 2.0+ moved divider
    try:
        from kivymd.uix.expansionpanel import MDDivider
    except ImportError:
        # Fallback: create a simple divider replacement
        from kivy.uix.widget import Widget
        class MDDivider(Widget):
...
# Handle loading client.kv from inside ZIP (when running from .apworld) or from file system
import sys
import os

kv_path = Utils.local_path("data", "client.kv")
logging.info(f"Attempting to load client.kv from: {kv_path}")

# Check if we're running from inside a ZIP file (.apworld)
if not os.path.exists(kv_path):
    logging.info(f"File not found at {kv_path}, trying to load from package data...")
    try:
        # Try to load from package data using pkgutil
        import pkgutil
        kv_data = pkgutil.get_data(__name__.split('.')[0], "data/client.kv")
        if kv_data:
            logging.info("Successfully loaded client.kv from package data, loading as string...")
            Builder.load_string(kv_data.decode('utf-8'))
        else:
            raise RuntimeError("Could not load client.kv from package data")
    except Exception as e:
        logging.error(f"Failed to load client.kv: {e}")
        raise
else:
    logging.info(f"Loading client.kv from file: {kv_path}")
